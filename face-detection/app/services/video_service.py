...
def _available_cores() -> int:
    """Сколько ядер нам доступно.

    ``os.cpu_count()`` возвращает все ядра машины и не видит ограничения
    контейнера: в контейнере, привязанном к четырём ядрам, он всё равно
    скажет восемь. ``sched_getaffinity`` видит привязку (``cpuset`` в
    compose), поэтому пул сам подстроится, если границу поменяют.

    Привязку он видит, а квоту (``cpus``) — нет. Если перейдут на квоту,
    размер пула придётся задавать вручную: потоков больше, чем ядер, под
    квотой не просто бесполезны, а вредны — когда бюджет окна исчерпан,
    все потоки замирают до его конца.
    """
    try:
        return len(os.sched_getaffinity(0))
    except AttributeError:
        # Вне Linux отсутствует; для локальной разработки неважно.
...
_executor = ThreadPoolExecutor(
    max_workers=max(1, _available_cores()),
