...
Sabab: fakultet, kafedra, mutaxassislik, guruh, oʻquv reja va fan EPOS/HEMIS
maʼlumoti — ularni platformada yaratish, oʻzgartirish, oʻchirish ham, yashirish
ham qoldirilmadi. Jadvallardagi ``is_hidden`` ustuni ataylab bazada qolgan
(qiymati hamma joyda ``false``) va modellardagi ``HideableMixin`` ham joyida:
aks holda keyingi ``alembic autogenerate`` ustunni tashlashni taklif qilardi.

Qaytarish uchun: shu fayl, repozitoriylardagi ``apply_visibility`` chaqiruvlari,
sxemalardagi ``include_hidden``, routerlardagi ``PATCH /{id}/visibility`` va
frontenddagi ``VisibilityControls`` kommentdan chiqariladi.
"""
