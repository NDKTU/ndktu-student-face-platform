...
async def test_list_groups_filters_by_speciality_and_exposes_catalog_fields(
    auth_client, async_db, make_speciality, test_group, test_kafedra
):
    from app.modules.organization_structure.model import Group

    # POST /speciality/ kommentga olindi (EPOS maʼlumoti) — qator repository orqali yaratiladi.
    speciality_id = (await make_speciality("Information systems", test_kafedra["id"]))["id"]
...
@pytest.mark.asyncio
async def test_list_groups_filters_course_and_shape_on_server(auth_client, async_db, test_faculty):
    """Kurs va ta'lim shakli filtri butun baza bo'yicha ishlaydi.

    Ilgari bu ikkovi frontda, ochilgan sahifaning ichida qo'llanardi:
    «1-kurs» tanlansa, jadvalda 15 qatordan bir nechtasi qolar, «jami» esa
    baribir hammasini ko'rsatar va sahifalarning ko'pi bo'm-bo'sh chiqardi.
    Shuning uchun bu yerda `total` ham tekshiriladi — aynan u yolg'on
    gapirardi.
    """
    from app.modules.organization_structure.model import Group
    ...
    async_db.add_all(
        [
            Group(name="F1-24", faculty_id=test_faculty["id"], course=1, education_shape="Kunduzgi"),
            Group(name="F2-24", faculty_id=test_faculty["id"], course=2, education_shape="Kunduzgi"),
            Group(name="F3-24", faculty_id=test_faculty["id"], course=1, education_shape="Kechki"),
        ]
    )
    await async_db.commit()

    by_course = await auth_client.get("/group/", params={"course": 1})
    assert by_course.status_code == 200
    assert by_course.json()["total"] == 2
    assert {g["name"] for g in by_course.json()["groups"]} == {"F1-24", "F3-24"}

    # Registr ahamiyatsiz: EPOS bir xil shaklni turlicha yozadi.
    by_shape = await auth_client.get("/group/", params={"education_shape": "kunduzgi"})
    assert by_shape.json()["total"] == 2
    assert {g["name"] for g in by_shape.json()["groups"]} == {"F1-24", "F2-24"}

    both = await auth_client.get("/group/", params={"course": 1, "education_shape": "Kechki"})
    assert both.json()["total"] == 1
    assert both.json()["groups"][0]["name"] == "F3-24"


@pytest.mark.asyncio
async def test_list_groups_sorts_on_server_across_pages(auth_client, async_db, test_faculty):
    """Saralash butun tanlov bo'yicha, sahifa ichida emas.

    Front sahifaga kelgan qatorlarni saralaganida, ikkinchi sahifa birinchidan
    kichik nom bilan boshlanishi mumkin edi — tartib har sahifada qaytadan
    boshlanardi.
    """
    from app.modules.organization_structure.model import Group
    ...
    async_db.add_all(
        [
            Group(name="B-guruh", faculty_id=test_faculty["id"], student_count=5),
            Group(name="A-guruh", faculty_id=test_faculty["id"], student_count=30),
            Group(name="C-guruh", faculty_id=test_faculty["id"], student_count=17),
        ]
    )
    await async_db.commit()

    first = await auth_client.get("/group/", params={"sort_by": "name", "order": "asc", "limit": 2, "page": 1})
    second = await auth_client.get("/group/", params={"sort_by": "name", "order": "asc", "limit": 2, "page": 2})
    names = [g["name"] for g in first.json()["groups"]] + [g["name"] for g in second.json()["groups"]]
    assert names == sorted(names)

    by_count = await auth_client.get("/group/", params={"sort_by": "student_count", "order": "desc"})
    counts = [g["student_count"] or 0 for g in by_count.json()["groups"]]
